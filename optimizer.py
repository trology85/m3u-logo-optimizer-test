...
def download_one(url: str, target: Path, timeout: int, max_bytes: int) -> Dict[str, object]:
    if not is_remote_http_url(url):
        return {'ok': False, 'url': url, 'path': str(target), 'error': 'skipped_invalid_url'}
...
def optimize(
        ...
        for raw in src:
            ...
            if line.startswith('#EXTINF'):
                ...
                if logo_url:
                    ...
                    is_remote_logo = is_remote_http_url(logo_url)
                    if is_remote_logo:
                        ...
                        # Bazı listelerde tvg-logo="noimage.png" gibi yerel/göreli değerler var.
                        # Bunlar indirilecek URL değildir; dönüşüm bozulmasın diye JSON'a işaretlenir.
                        local_file = ''
                        stats['invalidLogoRefs'] = int(stats.get('invalidLogoRefs', 0)) + 1
                    ...
                    item = items.setdefault(logo_id, {
                        ...
                        'usedCount': 0,
                        'downloaded': False,
                        'sourceType': 'remote' if is_remote_logo else 'invalid-or-relative',
                    })
    ...
    stats['uniqueLogoUrls'] = len(set(used_ids.values()))
    stats['uniqueRemoteLogoUrls'] = len(url_to_local)
